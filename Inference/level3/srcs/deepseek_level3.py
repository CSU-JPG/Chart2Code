...
# Ensure deepseek_vl is installed and in the Python path
try:
    from deepseek_vl.models import VLChatProcessor, MultiModalityCausalLM
    from deepseek_vl.utils.io import load_pil_images
    from transformers import AutoModelForCausalLM
except ImportError:
    print("❌ Error: 'deepseek_vl' library not found. Please ensure it is installed and accessible.")
    exit(1)
...
# Note: Adjust this path if your model is stored elsewhere relative to the script.
...
def generate_code(excel_data_str: str, style_image_path: str, task_instruction_text: str, model, processor) -> str:
    """Generates code using the DeepSeek-VL model based on the provided data, image, and instructions."""
    try:
        final_prompt = PROMPT_TEMPLATE.format(task_instructions=task_instruction_text, excel_data_string=excel_data_str)
        
        # Build the conversation structure for DeepSeek-VL
        conversation = [
            {"role": "User", "content": f"<image_placeholder>{final_prompt}", "images": [style_image_path]},
...
def main():
    # 1. Argument Parsing
    parser = argparse.ArgumentParser(description="Generate Python code from JSON tasks using DeepSeek-VL.")
    ...
    try:
        ...
        print(f"\n--- ✅ Batch Processing Complete (Model: DeepSeek-VL) ---")

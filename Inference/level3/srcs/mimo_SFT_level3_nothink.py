# This script generates Python code for data visualization tasks using the MiMo-VL model.
...
# Note: This is an example ID. Please adjust if the official Hub ID is different.
HUB_MODEL_ID = "XiaomiMiMo/MiMo-VL-7B-SFT-2508"

# 2. Path to the local model (for 'local' loading mode)
script_dir = os.path.dirname(os.path.abspath(__file__))
LOCAL_MODEL_PATH = os.path.abspath(os.path.join(script_dir, '..', '..', 'models', 'MiMo-VL-7B-SFT-2508'))
# ===================================================================================
...
def generate_code(excel_data_str: str, style_image_path: str, task_instruction_text: str, model, processor) -> str:
    """Generates code using the MiMo-VL model based on the provided data, image, and instructions."""
    try:
        ...
        with torch.no_grad():
            generated_ids = model.generate(
                **inputs, **INFERENCE_PARAMS, pad_token_id=processor.tokenizer.pad_token_id
            )
...
def main():
    # 1. Argument Parsing
    parser = argparse.ArgumentParser(description="Generate Python code from JSON tasks using MiMo-VL.")
    ...
    try:
        model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
            model_to_load, torch_dtype="auto", device_map="balanced"
        )
    ...
    try:
        ...
        print(f"\n--- ✅ Batch Processing Complete (Model: MiMo-VL) ---")

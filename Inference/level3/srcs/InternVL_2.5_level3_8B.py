# Description:
# This script generates Python code for data visualization tasks using the InternVL-2.5-8B model.
...
def split_model_for_devices(model_name_or_path):
    device_map = {}
    world_size = torch.cuda.device_count()
    model_name = os.path.basename(model_name_or_path)
    num_layers = {'InternVL2_5-8B': 32, 'InternVL2_5-38B': 64}.get(model_name, 32)
    ...
    for i, num_layer in enumerate(num_layers_per_gpu_list):
        for j in range(num_layer):
            if layer_cnt >= num_layers: continue
...
def main():
    # 1. Argument Parsing
    parser = argparse.ArgumentParser(description="Generate Python code from JSON tasks using InternVL-2.5-8B.")
    ...
    try:
        device_map = split_model_for_devices(model_to_load)
    ...
    try:
        ...
        print(f"\n--- ✅ Batch Processing Complete (Model: InternVL-2.5-8B) ---")

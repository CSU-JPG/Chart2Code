# This script generates Python code for data visualization tasks using the Ark API.
...
# ===================================================================================
# API Configuration (Loaded from environment variables in main)
# ===================================================================================
# Environment variables expected:
# ARK_API_KEY: The API key for authentication.
# ARK_API_BASE_URL: (Optional) The base URL for the API endpoint.
# ARK_MODEL_NAME: (Optional) The model identifier to use for generation.
# ===================================================================================
load_dotenv()
...
INFERENCE_PARAMS = {
    ...
    "top_p": 0.9,
    "thinking": {
        "type": "disabled"  # Disable deep thinking capability
    }
}
...
def generate_code(excel_data_str: str, style_image_path: str, task_instruction_text: str, client, API_MODEL) -> str:
    """Generates code using the Ark API."""
    try:
        ...
        if response.choices and response.choices[0].message.content:
            ...
            tqdm.write(f"   - Warning: API returned an empty response for task {os.path.basename(style_image_path)}")
            return None
...
def main():
    # 1. Argument Parsing
    parser = argparse.ArgumentParser(description="Generate Python code from JSON tasks using the Ark API.")
    ...
    # 3. Configure API Client from Environment Variables
    try:
        api_key = os.getenv('ARK_API_KEY'),
        base_url = os.getenv('ARK_BASE_URL')

        if not api_key:
            raise ValueError("ARK_API_KEY must be set in the environment.")

...
def generate_code(excel_data_str: str, style_image_path: str, task_instruction_text: str, model, tokenizer) -> str:
    """Generates code using the GLM model based on the provided data, image, and instructions."""
    try:
        final_prompt = PROMPT_TEMPLATE.format(task_instructions=task_instruction_text, excel_data_string=excel_data_str)
        
        style_image = Image.open(style_image_path).convert('RGB')
        
        messages = [{"role": "user", "image": style_image, "content": final_prompt}]
...
def main():
    # 1. Argument Parsing
    parser = argparse.ArgumentParser(description="Generate Python code from JSON tasks using GLM-4V-9B.")

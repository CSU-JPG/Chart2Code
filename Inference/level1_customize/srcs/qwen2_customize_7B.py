...
# uses a dynamically generated prompt with the Qwen2-VL model to generate Python code,
...
HUB_MODEL_ID = "Qwen/Qwen2-VL-7B-Instruct"

# 2. Path to the local model (for 'local' loading)
script_dir = os.path.dirname(os.path.abspath(__file__))
LOCAL_MODEL_PATH = os.path.abspath(os.path.join(script_dir, '..', '..', 'models', 'Qwen2-VL-7B-Instruct'))
# ===================================================================================
...
def main():
    ...
    try:
        processor = AutoProcessor.from_pretrained(model_to_load, max_pixels=1280 * 28 * 28, use_fast=True)
        model = Qwen2VLForConditionalGeneration.from_pretrained(model_to_load, torch_dtype=torch.float16, device_map="balanced").eval()
    ...
    # This is a crucial step to handle potentially malformed JSON keys.
    ...
    try:
        with open(LOG_CSV_PATH, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            for task in tqdm(tasks, desc="Processing Tasks"):
                ...
                gt_image_path = task.get("GT image") # <-- NEW: Get the GT image path for naming

                # Validate that all required file paths were found in the JSON task
                if not all([relative_image_path, relative_instruction_path, relative_data_path, gt_image_path]): # <-- MODIFIED: Check for gt_image_path
                ...
                # --- MODIFIED: Construct output path based on GT image filename ---
                # Extract the base filename from the GT image path (e.g., "table_1_instruction_2.png")
                base_filename_with_ext = os.path.basename(gt_image_path)
                # Split the filename into its root and extension and create the new Python filename
        ...
        print(f"Succeeded: {processed_count - failure_count} | Failed: {failure_count}")

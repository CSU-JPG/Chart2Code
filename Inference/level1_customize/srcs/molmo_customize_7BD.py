...
# uses a dynamically generated prompt with the Molmo model to generate Python code,
...
# The Molmo model does not require a separate utility file like qwen_vl_utils.
# Its image processing is handled by its specific processor.

# ===================================================================================
# Model Configuration
# ===================================================================================
# 1. Model ID on Hugging Face Hub (for 'hub' loading)
HUB_MODEL_ID = "allenai/Molmo-7B-D-0924" # Example Hub ID for Molmo
...
INFERENCE_PARAMS = {
    ...
    "max_new_tokens": 8192,
    "do_sample": True,
    "stop_strings": "<|endoftext|>" # Molmo-specific parameter
}
...
def extract_python_code(raw_text: str) -> str:
    ...
    if match:
        ...
        if match:
            return match.group(1).strip()
    return "" # Return empty if no code block is found
...
"""
        ...
        # Create GenerationConfig object from our parameters dictionary
        generation_config = GenerationConfig(**INFERENCE_PARAMS)

        # Process inputs using the Molmo processor
...
def main():
    # --- 1. Argument Parsing ---
    parser = argparse.ArgumentParser(description="Generate Python code from a JSON task file using the Molmo model.")

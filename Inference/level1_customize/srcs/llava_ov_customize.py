...
# uses a dynamically generated prompt with the Llava-Onevision model to generate Python code,
...
HUB_MODEL_ID = "llava-hf/llava-onevision-qwen2-7b-ov-hf"
...
def generate_code_for_image(image_path: str, instruction_text: str, data_text: str, model, processor) -> str:
    ...
    try:
        prompt_text = f"""You are a Python developer proficient in data visualization, with expertise in using libraries such as Matplotlib, NetworkX, Seaborn, and others. Your task is to generate Python code that can perfectly reproduce a plot based on a reference image, a natural language instruction, and the corresponding data.
...
"""
        ...
        # Build the conversation with the dynamic prompt
        conversation = [{
            "role": "user",
            "content": [

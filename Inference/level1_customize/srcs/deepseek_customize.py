...
# uses a dynamically generated prompt with the DeepSeek-VL model to generate Python code,
...
# --- DeepSeek-VL Specific Imports ---
# Ensure the 'deepseek_vl' library is installed (`pip install deepseek-vl`)
try:
    from deepseek_vl.models import VLChatProcessor, MultiModalityCausalLM
    from deepseek_vl.utils.io import load_pil_images
except ImportError:
    print("❌ Error: 'deepseek_vl' library not found. Please install it using: pip install deepseek-vl")
    exit(1)
...
def generate_code_for_image(image_path: str, instruction_text: str, data_text: str, model, vl_chat_processor, tokenizer) -> str:
    """Generates code for a given image using the specific DeepSeek-VL conversation format."""
    try:
        # 1. Dynamically create the complete prompt
...
```"""
        ...
        # 2. Build the conversation format required by DeepSeek-VL
        conversation = [
            {"role": "User", "content": f"<image_placeholder>{prompt}", "images": [image_path]},
            {"role": "Assistant", "content": ""}
        ]

        # 3. Load images and prepare inputs using the model's processor
        pil_images = load_pil_images(conversation)
        prepare_inputs = vl_chat_processor(
            conversations=conversation, images=pil_images, force_batchify=True
        ).to(model.device)

        # 4. Generate the response using the model-specific method
        with torch.no_grad():
...
def main():
    ...
    # --- 4. Load Model and Processor (DeepSeek-VL specific) ---
    print(f"Current device: {DEVICE}")
    try:
        vl_chat_processor: VLChatProcessor = VLChatProcessor.from_pretrained(model_to_load, trust_remote_code=True)
        tokenizer = vl_chat_processor.tokenizer
        model: MultiModalityCausalLM = AutoModelForCausalLM.from_pretrained(
            model_to_load, trust_remote_code=True
        ).to(torch.bfloat16).to(DEVICE).eval()
        print("✅ DeepSeek-VL model and processor loaded successfully!")
    except Exception as e:
    ...
    try:
        with open(LOG_CSV_PATH, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            for task in tqdm(tasks, desc="Processing Tasks"):
                ...
                raw_generated_text = generate_code_for_image(image_path, instruction_text, data_text, model, vl_chat_processor, tokenizer)

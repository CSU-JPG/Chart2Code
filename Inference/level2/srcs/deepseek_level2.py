...
# and edit instructions to generate Python code with the DeepSeek-VL model, saves the result, and logs the process.
...
def generate_code_for_image(image_path: str, instruction_text: str, model, vl_chat_processor, tokenizer) -> str:
    """Generates code for a given image using DeepSeek-VL."""
    try:
        ...
        conversation = [
            {
                "role": "User",
                "content": f"<image_placeholder>{full_prompt}",
                "images": [image_path]
            },
            {
                "role": "Assistant",
...
def main():
    ...
    try:
        vl_chat_processor = VLChatProcessor.from_pretrained(model_to_load, trust_remote_code=True)
    ...
    try:
        with open(LOG_CSV_PATH, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            for task in tqdm(tasks, desc="Processing Tasks"):
                ...
                raw_generated_text = generate_code_for_image(image_path, instruction_text, model, vl_chat_processor, tokenizer)

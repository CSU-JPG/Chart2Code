...
# and edit instructions to generate Python code via the Gemini API, saves the result, and logs the process.
...
INFERENCE_PARAMS = {
    "temperature": 0.1,
    "max_tokens": 8192  # Adjusted for consistency, can be higher if needed
}
...
def generate_code_for_image(image_path: str, instruction_text: str, client) -> str:
    """Generates code via the OpenAI-compatible API for Gemini."""
    try:
...
def main():
    # --- 1. Argument Parsing ---
    parser = argparse.ArgumentParser(description="Generate Python code from a JSON task file using the Gemini API.")

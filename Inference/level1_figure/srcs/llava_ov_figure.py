...
# (data and style) to generate Python code with the Llava-OneVision model, saves the result, and logs the process.
...
HUB_MODEL_ID = "llava-hf/llava-onevision-qwen2-7b-ov-hf" # Example Hub ID for this model

# 2. Path to the local model (for 'local' loading)
script_dir = os.path.dirname(os.path.abspath(__file__))
LOCAL_MODEL_PATH = os.path.abspath(os.path.join(script_dir, '..', '..', 'models', 'llava-onevision-qwen2-7b-ov-hf'))
# ===================================================================================
...
def generate_code_for_image(data_image_path: str, style_image_path: str, task_instruction_text: str, model, processor) -> str:
    ...
    try:
        ...
        # Llava-OneVision uses a specific conversation format
        conversation = [{
            "role": "user",
            "content": [
                {"type": "text", "text": final_prompt},
                {"type": "text", "text": "\n\nThis is the data source image:"},
                {"type": "image"}, # Placeholder for data image
                {"type": "text", "text": "\n\nThis is the style reference image:"},
                {"type": "image"}, # Placeholder for style image
            ],
        }]
        ...
        # The processor expects a list of images corresponding to the image placeholders
        inputs = processor(
...
def main():
    # --- 1. Argument Parsing ---
    parser = argparse.ArgumentParser(description="Generate Python code from a JSON task file using Llava-OneVision.")

...
# to generate Python code with the Kimi-VL model, saves the result, and logs the process.
...
HUB_MODEL_ID = "moonshotai/Kimi-VL-A3B-Instruct" # NOTE: Example, use the correct ID for your model

# 2. Path to the local model (for 'local' loading)
script_dir = os.path.dirname(os.path.abspath(__file__))
LOCAL_MODEL_PATH = os.path.abspath(os.path.join(script_dir, '..', '..', 'models', 'Kimi-VL-A3B-Instruct'))
# ===================================================================================
...
def generate_code_for_image(data_image_path: str, style_image_path: str, task_instruction_text: str, model, processor) -> str:
    """Generates code by combining a prompt template with instructions and processing two images with Kimi-VL."""
    try:
        ...
        # Construct the messages list with placeholders for two images
        messages = [
            {"role": "user", "content": [
        ...
        # Pass the list of images in the correct order
        inputs = processor(
...
def main():
    # --- 1. Argument Parsing ---
    parser = argparse.ArgumentParser(description="Generate Python code from a JSON task file using Kimi-VL.")
    ...
    try:
        ...
        # Ensure pad_token is set to avoid generation errors

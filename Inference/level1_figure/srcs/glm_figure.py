...
# (data and style) to generate Python code with the GLM-4V-9B model, saves the result, and logs the process.
...
def generate_code_for_image(data_image_path: str, style_image_path: str, task_instruction_text: str, model, tokenizer) -> str:
    """Generates code using the GLM-4V model with the user-specified message format."""
    try:
        ...
        # The 'messages' variable is constructed exactly as specified in the original script.
        messages = [
            {
                "role": "user",
                "image": data_image,
                "content": f"{final_prompt}\n\nThe image above is the data source image.",
                "image": style_image,
                "content": "The image above is the reference style image. Now, considering all instructions and both images, generate the complete Python code."
            }
        ]
        
        # Process inputs using the GLM tokenizer
...
def main():
    # --- 1. Argument Parsing ---
    parser = argparse.ArgumentParser(description="Generate Python code from a JSON task file using GLM-4V-9B.")

...
# (data and style) to generate Python code with the Qwen3-VL model, saves the result, and logs the process.
...
HUB_MODEL_ID = "Qwen/Qwen3-VL-30B-A3B-Thinking" # Example, please verify the correct Hub ID if needed

# 2. Path to the local model (for 'local' loading)
script_dir = os.path.dirname(os.path.abspath(__file__))
LOCAL_MODEL_PATH = os.path.abspath(os.path.join(script_dir, '..', '..', 'models', 'Qwen3-VL-30B-A3B-Thinking'))
# ===================================================================================
...
def generate_code_for_image(data_image_path: str, style_image_path: str, task_instruction_text: str, model, processor) -> str:
    ...
    try:
        ...
        # Build the messages list with the full prompt and two clearly labeled images.
        # This structure follows the Qwen3-VL API for multi-modal inputs.
        messages = [
            {"role": "user", "content": [
        ...
        # Prepare inputs using the Qwen3-VL API
...
def main():
    # --- 1. Argument Parsing ---
    parser = argparse.ArgumentParser(description="Generate Python code from a JSON task file using Qwen3-VL.")
    ...
    print(f"Current device: {DEVICE} (using device_map)")
    try:
        processor = AutoProcessor.from_pretrained(model_to_load)
        model = AutoModelForImageTextToText.from_pretrained(
            model_to_load,
            dtype=torch.float16,
            device_map="balanced"  # Using balanced for multi-GPU as in the original script
        ).eval()

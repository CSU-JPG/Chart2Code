...
# (data and style) to generate Python code via the GPT-5 API, saves the result, and logs the process.
...
def generate_code_for_image(data_image_path: str, style_image_path: str, task_instruction_text: str, client) -> str:
    """Generates code via an OpenAI-compatible API."""
    try:
...
def main():
    # --- 1. Argument Parsing ---
    parser = argparse.ArgumentParser(description="Generate Python code from a JSON task file using the GPT-5 API.")

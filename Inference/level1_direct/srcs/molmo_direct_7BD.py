# This script reads a JSON file to get image paths, uses a predefined prompt template with the Molmo model
# to generate Python code based on the image, saves the result to a specified directory, and logs the process.
import os
import torch
...
HUB_MODEL_ID = "allenai/Molmo-7B-D-0924"
...
# --- Global Variables ---
# processor and model will be loaded in the main function
processor = None
model = None
...
# Following the Molmo example, we use a GenerationConfig object to encapsulate inference parameters.
INFERENCE_CONFIG = GenerationConfig(
    temperature=0.1,
    top_p=0.9,
    max_new_tokens=4096,
    do_sample=True,
    stop_strings="<|endoftext|>" # Critical parameter to tell the model where to stop
)


# --- Model-Specific Helper Functions for Molmo (Do not change) ---
def load_image(image_path, max_size=(512, 512)):
    """
    Image loading and preprocessing function adapted from the Molmo example.
    """
    try:
    ...
    img = img.convert("RGB")
    # Resize the image to conform to model recommendations
...
def generate_code_for_image(image_path: str) -> str:
    """Generates code for a given image using the global prompt template and Molmo's API."""
    try:
        ...
        # 2. Use processor to handle image and text
        inputs = processor.process(
            images=[image],
            text=PROMPT_TEMPLATE
        )

        # 3. Move inputs to the correct device and add a batch dimension
        inputs = {k: v.to(model.device).unsqueeze(0) for k, v in inputs.items()}

        # 4. Execute model generation
        ...
        # Get only the newly generated part of the tokens
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate Python code for images specified in a JSON file using the Molmo model.")
    ...
    print(f"Loading Molmo model and processor from '{model_to_load}'...")
    try:

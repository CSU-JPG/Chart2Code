# Description:
# This script reads a JSON file to get image paths, uses a predefined prompt template with the InternVL-3.5-38B model
# to generate Python code based on the image, saves the result to a specified directory, logs the process,
# and incorporates advanced dynamic image preprocessing for high-resolution inputs.
...
def generate_code_for_image(image_path: str) -> str:
    ...
    try:
        # InternVL requires the <image> placeholder in the prompt
        question = f"{PROMPT_TEMPLATE}\n<image>"

        image = Image.open(image_path).convert('RGB')
        pixel_values = process_image(image).to(torch.float16).cuda()
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate Python code for images specified in a JSON file using the InternVL-3.5-38B model.")
    ...
    print(f"Loading InternVL-3.5-38B model and tokenizer from '{model_to_load}'...")
    try:
    ...
    try:
        ...
        print(f"\n--- ✅ Batch processing complete (Model: InternVL-3.5-38B) ---")

# This script reads a JSON file to get image paths, generates Python code for the images using the OpenAI API,
...
OPENAI_TIMEOUT = float(os.getenv('OPENAI_TIMEOUT', 600.0))
# The OPENAI_API_KEY will be automatically read from the .env file by the openai library
# ===================================================================================
...
# --- Core Inference Function (Adapted for API) ---
def generate_code_for_image(image_path: str) -> str:
    """Generates code for a given image using the OpenAI API."""
    try:
...
def main():
    ...
    global client
    print(f"Initializing OpenAI API client for model '{API_MODEL_ID}'...")
    try:
        ...
        if not OPENAI_API_URL or not client.api_key:
             raise ValueError("OPENAI_API_URL or OPENAI_API_KEY is not configured in the .env file.")
    ...
    try:
        ...
        # Assumes the script is in a subdirectory, e.g., benchmark/Inference/level1_direct/srcs/

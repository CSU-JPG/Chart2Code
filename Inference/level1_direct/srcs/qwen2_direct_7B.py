# This script reads a JSON file to get image paths, generates Python code for the images using a predefined prompt template and the Qwen2-VL-7B-Instruct model,
...
# This utility function is specific to the Qwen2-VL model and is required for processing visual inputs.
...
def generate_code_for_image(image_path: str) -> str:
    ...
    try:
        # Build messages list using the image and the prompt template
        messages = [
            {
                "role": "user",
                "content": [
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate Python code for images specified in a JSON file using the Qwen2-VL-7B-Instruct model.")
    ...
    print(f"Loading Qwen2-VL-7B-Instruct model and processor from '{model_to_load}'...")
    try:
    ...
    try:
        ...
        print(f"\n--- ✅ Batch processing complete (Model: Qwen2-VL-7B-Instruct) ---")

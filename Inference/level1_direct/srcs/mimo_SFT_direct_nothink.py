# This script reads a JSON file to get image paths, uses a predefined prompt template with the MiMo-VL model
...
INFERENCE_PARAMS = {
    "temperature": 0.1,
    "top_p": 0.95,
    "max_new_tokens": 4096, # Adjusted for consistency
    "do_sample": True
}
...
def extract_python_code(raw_text: str) -> str:
    ...
    """
    # First, try to match a block labeled with 'python'
    match = re.search(r"```python\s*\n(.*?)\n```", raw_text, re.DOTALL)
    if match:
        return match.group(1).strip()
    
    # If not found, fall back to a generic code block
    match = re.search(r"```\s*\n(.*?)\n```", raw_text, re.DOTALL)
    if match:
        return match.group(1).strip()
        
    # Expected behavior: If no code block is found, an empty string must be returned
    return ""

# --- Core Inference Function (Do not change) ---
def generate_code_for_image(image_path: str) -> str:
    """Generates code for a given image using the global prompt template with the MiMo-VL model."""
    try:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate Python code for images specified in a JSON file using the MiMo-VL model.")
    ...
    print(f"Loading MiMo-VL model and processor from '{model_to_load}'...")
    try:
        ...
        processor = AutoProcessor.from_pretrained(model_to_load, trust_remote_code=True, max_pixels=4096 * 28 * 28)
